...
# ---------------------------------------------------------------- emit schematic
def build(sim, sections_in=None, sim_sections_in=None, base_project=None,
          base_title=None, directives_in=None):
    root_uuid = uid()
    bp = base_project or "tank-driver-recovery"
    bt = base_title or "GAS Rev A - Tank Driver Recovery"
    project = bp + "-sim" if sim else bp
    body = []
    ...
    title = bt + (" - SIMULATION TESTBENCH" if sim else " Board")
    ...
    base_secs = sections_in if sections_in is not None else SECTIONS
    extra_secs = sim_sections_in if sim_sections_in is not None else SIM_SECTIONS
    sections = base_secs + (extra_secs if sim else [])
    needed, comps = {}, []
    def skip_in_sim(c):
